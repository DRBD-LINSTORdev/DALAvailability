...
class PerformCreateResourceTask():
    ...
    """
    def __init__(self):
        self.obj_config = config_file.ConfFile('../config.yaml')
        self.yaml_info_list = self.obj_config.read_yaml()
        self.obj_diskless = Ssh(name=self.yaml_info_list['node'][0]['name']
                                ,ip=self.yaml_info_list['node'][0]['ip']
                                ,username="root"
                                ,password=self.yaml_info_list['node'][0]['password'])
        self.obj_diskful01 = Ssh(name=self.yaml_info_list['node'][1]['name']
                                ,ip=self.yaml_info_list['node'][1]['ip']
                                ,username="root"
                                ,password=self.yaml_info_list['node'][1]['password'])
        self.obj_diskful02 = Ssh(name=self.yaml_info_list['node'][2]['name']
                                ,ip=self.yaml_info_list['node'][2]['ip']
                                ,username="root"
    ...
    def check_nodeid_and_volume(self):
        ...
        info = self.obj_diskful01.exec_command(f'drbdsetup status giresource -vs')
    ...
    def step1(self):
        ...
        try:
            ...
            self.obj_diskful01.exec_command('linstor resource-definition create giresource')
            self.obj_diskful01.exec_command(f'linstor volume-definition create giresource 5G')
        ...
        self.obj_diskful01.exec_command(f'linstor resource create {self.yaml_info_list["node"][1]["name"]} giresource --storage-pool sptests')
        print(f'节点{self.yaml_info_list["node"][1]["name"]}的diskful资源尝试创建')
        self.obj_diskful01.exec_command(f'linstor resource create {self.yaml_info_list["node"][2]["name"]} giresource --storage-pool sptests')
        print(f'节点{self.yaml_info_list["node"][2]["name"]}的diskful资源尝试创建')
    ...
    def step2(self):
        ...
        info = self.check_resource_status(self.obj_diskful01)
        node1_name = self.yaml_info_list['node'][1]['name']
        node2_name = self.yaml_info_list['node'][2]['name']
        try:
    ...
    def step3(self):
        ...
        GI_info1 = self.obj_diskful01.exec_command(node1_GI_query_cmd)

        GI_info2 = self.obj_diskful02.exec_command(node2_GI_query_cmd)
    ...
    def step4(self):
        ...
        state = False
        cmd = f'linstor resource create {self.yaml_info_list["node"][0]["name"]} giresource --diskless'
        try:
            self.obj_diskless.exec_command(cmd)
            print("diskless创建成功")
            state = True
        except:
...
class SyncCheck():
    ...
    def linstor_sync_check(self):
        ...
        node1_name = self.yaml_info_list['node'][1]['name']
        node2_name = self.yaml_info_list['node'][2]['name']
        info = self.obj_diskful01.exec_command('linstor r l | grep giresource')
...
class DdWriteData(SyncCheck):
    ...
    def get_devicename(self):
        info = self.obj_diskful01.exec_command("linstor r lv | grep giresource")
...
class DrbdNetworkOperation(SyncCheck):
    ...
    def down_interface(self):
        try:
            cmd = f'ifconfig {self.yaml_info_list["node"][1]["nic"]} down'
            self.obj_diskful01.exec_command(cmd)
            info = self.obj_diskful01.exec_command('nmcli device')
            test = re.findall(r'%s  ethernet  (connected)' % self.yaml_info_list["node"][1]["nic"], info)
            if test == []:
                print(f'{self.yaml_info_list["node"][1]["name"]}的网卡：{self.yaml_info_list["node"][1]["nic"]}已经关闭')
            else:
    ...
    def up_interface(self):
        try:
            cmd = f'nmcli device connect {self.yaml_info_list["node"][1]["nic"]}'
            self.obj_diskful01.exec_command(cmd)
            print(f'网卡：{self.yaml_info_list["node"][1]["nic"]}已经开启')
            state = True
        except:
...
class StopDdAndCheckGituple(SyncCheck):
    ...
    def stop_dd(self):
        print(f"停止diskless节点{self.yaml_info_list['node'][0]['name']}的写dd操作")
        try:
            process_info = self.obj_diskless.exec_command('ps -A | grep dd')
            test = re.findall(f'([\d]+) \?        \w\w:\w\w:\w\w dd',process_info)
            dd_pid = test[0]
            self.obj_diskless.exec_command(f'kill {dd_pid}')
            print("dd进程已终止")
            state = True
        except:
...
class NodeOperationMock(SyncCheck):
    ...
    def down_interface(self):
        try:
            cmd = f'ifconfig {self.yaml_info_list["node"][2]["nic"]} down'
            self.obj_diskful02.exec_command(cmd)
            print(f'{self.yaml_info_list["node"][2]["name"]}网卡：{self.yaml_info_list["node"][2]["nic"]}已经关闭')
            state = True
        except:
            logging.warning("网卡关闭失败")
            print("网卡关闭失败")
    ...
    def up_interface(self):
        try:
            cmd = f'nmcli device connect {self.yaml_info_list["node"][2]["nic"]}'
            self.obj_diskful02.exec_command(cmd)
            print(f'{self.yaml_info_list["node"][2]["name"]}网卡：{self.yaml_info_list["node"][2]["nic"]}已经开启')
            state = True
        except:
            logging.warning("网卡开启失败")
            print("网卡开启失败")
    ...
    def linstor_cluster_check(self):
        ...
        node1_name = self.yaml_info_list['node'][1]['name']
        node2_name = self.yaml_info_list['node'][2]['name']
        info = self.obj_diskful01.exec_command('linstor r l -p | grep giresource')
...
class NodeOperation(SyncCheck):
    def __init__(self):
        ...
        self.obj_diskful02 = Ssh(name=self.yaml_info_list['node'][2]['name']
                                ...
                                ,username="root"
                                ,password=self.yaml_info_list['node'][2]['password'])
        self.ip = self.yaml_info_list["phynode"][0]["ip"]
        self.username = self.yaml_info_list["phynode"][0]['username']
        self.password = self.yaml_info_list["phynode"][0]['password']
    ...
    def up_interface(self):
        try:
            ...
            while a is False:
                time.sleep(20)
                state2 = self.obj_diskless.exec_command(check_poweron_cmd)
...
class DeleteResource(SyncCheck):
    ...
    def start_up(self):
        ...
        try:
            ...
            check_result = self.obj_diskful01.exec_command(check_cmd)
